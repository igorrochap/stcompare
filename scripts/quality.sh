...
coverage() {
  ...
  if ! coverage_profile="$(mktemp "${TMPDIR:-/tmp}/stcompare-coverage.XXXXXX")"; then
...
architecture() {
  ...
  check_forbidden_import \
    ...
    "internal/bench must invoke stcompare through its CLI contract" || failed=1
  check_forbidden_import \
    stcompare/internal/comparison \
    stcompare/benchrecord \
    "internal/comparison must remain independent of benchmark records" || failed=1
  check_forbidden_import \
    stcompare/internal/comparison \
    stcompare/internal/scorecard \
    "internal/comparison must not depend on the joined scorecard" || failed=1
  check_forbidden_project_imports \
    stcompare/agentreport \
    "agentreport must remain a zero-project-dependency public contract" || failed=1
  check_forbidden_project_imports \
    stcompare/benchrecord \
    "benchrecord must remain a zero-project-dependency public contract" || failed=1
...
check_forbidden_import() {
  ...
  if ! imports="$(go list -f '{{join .Imports "\n"}}' "$package")"; then
    return 1
  fi
  if ! grep -Fqx "$forbidden_import" <<<"$imports"; then
    return 0
  fi

  printf '%s imports forbidden package %s: %s.\n' "$package" "$forbidden_import" "$reason" >&2
  return 1
}
...
check_forbidden_project_imports() {
  ...
  project_imports="$(grep '^stcompare/' <<<"$imports" || true)"
  if [[ -z "$project_imports" ]]; then
    return 0
  fi

  printf '%s has forbidden project dependencies: %s.\n' "$package" "$reason" >&2
  printf '%s\n' "$project_imports" >&2
  return 1
}
